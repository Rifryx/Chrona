...
async def start_chat(msg: Message, user: User, is_new_user: bool):
    if is_new_user:
        await msg.answer_photo(photo=get_photo("start"),
            caption=f"Привет, {user.first_name}! Рада тебя видеть тут, выбери часовой пояс и давай знакомиться",
...
async def pick_tz_start(call: CallbackQuery, user: User):
    ...
    if iana is None:
        ...
        return
    await SettingsService.create_sett(user.id, iana)
