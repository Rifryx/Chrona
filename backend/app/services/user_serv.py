...
class UserService:
    ...
    async def create_user(tg_id: int, username: str | None, first_name: str | None) -> User:
        async with session_factory() as session:
            new_user = User(telegram_id=tg_id, username=username, first_name=first_name, created_at=datetime.utcnow(), updated_at=datetime.utcnow())
    ...
    async def set_tz(tg_id: int, tz: str) -> None:
        async with session_factory() as session:
            await session.execute(update(User).where(User.telegram_id==tg_id).
